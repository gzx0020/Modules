...
#include "spi.h"
#include <stdio.h>
#include <stdlib.h> // 添加 malloc 所需的头文件
...
void ADS1256_Read_MultiChannels(uint8_t *channels, uint8_t num_channels, uint32_t sample_count, float *samples) {
    ...
    // 确保在命令模式
    ADS1256_SendCommand(ADS1256_CMD_SDATAC);
    
    for (sample_index = 0; sample_index < sample_count; sample_index++) {
        for (channel_index = 0; channel_index < num_channels; channel_index++) {
            // 设置通道
            ADS1256_Write(ADS1256_MUX, channels[channel_index]);
            ...
            // 读取数据
            ADS1256_Read_ADC_data(raw_data);
            
            // 转换为电压并存储
            samples[sample_index * num_channels + channel_index] = 
                Convert_ADC_to_Voltage(raw_data);
        }
    }
}